...
export const useApi = (url, config) => {
  ...
  const authorized = e => !(e && e.response && [401, 403].includes(e.response.status))
    ...
    if(data && data.isAxiosError) {
      ...
      if(authorized(error)) {
        ...
        return new Promise((resolve, reject) => {
          setRetry((retry) => () => {
            retry && retry();
            decoratedFetch(...args).then(resolve).catch(reject)
          });
          setLoginOpen(true);
        });
      }
  ...
  return [Object.assign({}, result, { loading: !authorized(error) || loading, error: authorized(error) ? error : null }), decoratedFetch];
}